...
Student Learning State & Telemetry Grounding (RAG):
- Current Concept Mastery Score: {progress.mastery_pct:.1f}%
- Retention Level: {progress.retention_pct:.1f}%
- Attempts Count: {progress.attempts_count}
- Quiz History: {quiz_telemetry_str}

Mastery of Prerequisite Concepts in the Dependency Graph:
{prereq_context_str}
{misconceptions_str}

INSTRUCTION: Adapt your teaching directly based on the prerequisite states and quiz mistakes.
- If they are struggling with prerequisites, feel free to refer to those concepts to help bridge their understanding.
- If concept mastery is low (<40%), explain with simple analogies, be extremely patient, and guide them step-by-step.
- If mastery is high (>70%), dive into advanced nuances, ask thought-provoking follow-ups, and keep explanations concise.
- If they have confident-incorrect answers listed above, address those specific misconceptions and help them correct their understanding.
